...
- [24.1美规](#241美规)
  ...
  - [24.1.3钢筋松弛](#2413钢筋松弛)
  - [24.1.4 活载](#2414-活载)
    - [24.1.4.1 设计汽车荷载](#24141-设计汽车荷载)
    - [24.1.4.2 多车道折减](#24142-多车道折减)
    - [24.1.4.3 荷载动力效应](#24143-荷载动力效应)
    - [24.1.4.4 车道支承反力和负弯矩](#24144-车道支承反力和负弯矩)
  - [24.1.5 混凝土检算](#2415-混凝土检算)
    - [24.1.5.1检算荷载组合](#24151检算荷载组合)
      - [检算荷载组合](#检算荷载组合)
      - [自动生成检算荷载组合](#自动生成检算荷载组合)
    - [24.1.5.2计算项目](#24152计算项目)
      - [正截面承载力](#正截面承载力)
      - [斜截面抗剪承载力](#斜截面抗剪承载力)
      - [正应力](#正应力)
      - [剪应力与主应力](#剪应力与主应力)
...
- [24.2英规](#242英规)
  ...
  - [24.2.3钢筋松弛](#2423钢筋松弛)
  - [24.2.4 活载](#2424-活载)
    - [24.2.4.1车俩荷载](#24241车俩荷载)
    - [24.2.4.2 多车道折减系数](#24242-多车道折减系数)
  - [24.2.5 混凝土检算](#2425-混凝土检算)
    - [24.2.5.1检算荷载组合](#24251检算荷载组合)
      - [检算荷载组合](#检算荷载组合-1)
      - [自动生成检算荷载组合](#自动生成检算荷载组合-1)
    - [24.2.5.2材料本构](#24252材料本构)
    - [24.2.5.3计算项目](#24253计算项目)
      - [正截面承载力](#正截面承载力-1)
      - [斜截面抗剪承载力](#斜截面抗剪承载力-1)
      - [正应力](#正应力-1)
      - [裂缝宽度](#裂缝宽度-1)
...
- 计算说明
  ...
  - **最小配筋率是否满足要求**
    ...
    $$
     M_r \geq \min \left\{ \begin{array}{l} 1.33M_{\text{设计}} \\ M_{cr} \end{array} \right\}
    $$
    $M_{\text{设计}}$：计算组合的设计力矩；

    $M_{cr}$：开裂弯矩，
    $$
    M_{cr} = \gamma_{3} \left[ \left( \gamma_{1} f_{r} + \gamma_{2} f_{cpe} \right) S_{c} - M_{dnc} \left( \frac{S_{c}}{S_{nc}} - 1 \right) \right] \quad (5.6.3.3-1)
    $$

    | 符号         | 定义                    | 单位     |
    | ---------- | --------------------- | ------ |
    | \$f\_r\$   | 混凝土断裂模量               | ksi    |
    | $f _{cpe}$ | 极端受拉纤维处仅由有效预应力产生的压应力  | ksi    |
    | \$S\_c\$   | 组合截面极端受拉纤维截面模量        | in³    |
    | $M _{dnc}$ | 作用在整体/非组合截面上的总未分解恒载力矩 | kip-in |
    | $S _{nc}$  | 整体/非组合截面极端受拉纤维截面模量    | in³    |
...
- 计算说明
  - 标称抗剪强度Vn计算
    ...
    *(**规范**公式 5.7.3.3-1 和 5.7.3.3-2)* &#x20;
    - $V_c$**：** 混凝土抗剪分量
      $$
      V_c = 0.0316\beta\lambda\sqrt{f'_c} b_v d_v \quad (规范5.7.3.3-3)
      $$
    - $V_s$**：** 钢筋抗剪分量
      $$
      V_s = \frac{A_v f_y d_v (\cot\theta + \cot\alpha) \sin\alpha}{s} \lambda_{\text{duct}} \quad (规范5.7.3.3-4)
      $$
      - $\lambda_{\text{duct}}$**：** 抗剪强度折减系数，该系数考虑了由于存在灌浆后张管道而导致的横向钢筋提供的抗剪强度的降低。对于未布线的后张拉管道，取1.0，并减小腹板或法兰宽度，以考虑未布线管道的存在。
        $$
        \lambda_{\text{duct}} = 1 - \delta \left( \frac{\Phi_{\text{duct}}}{b_w} \right)^2 \quad (规范5.7.3.3-5)
        $$
        **程序中用户在分析设置中自行输入。**
  ...
  - 抗剪参数β和θ
    ...
    $$
     \beta = \frac{4.8}{(1+750ε_s)}   (规范5.7.3.4.2-1)
    $$
    θ的值可通过以下方程式确定：
    $$
    \theta = 29 + 3500ε_s (规范5.7.3.4.2-3)
    $$
...
- 计算说明
  ...
  - 正常使用极限状态应力计算
    - 正常使用极限状态应力限值
      ...
      | --------------- | ------- | ------------------------ | -------------- | -------------- |
      | **不允许开裂预应力构件**​ | 施工阶段    | $0.65f' _{ci}$           | **分析设置中自行设置**​ | $0.90f _{ptk}$ |
      |                 | 成桥      | $0.45f' _{ck}$           | \\-            | \\-            |
      |                 | 使用组合I   | \$0.60\phi\_w f'\_{ck}\$ | \\-            | $0.80f _{ptk}$ |
      |                 | 使用组合III | \\-                      | **分析设置中自行设置**​ | $0.80f _{ptk}$ |
      |                 | 其他使用组合  | \\-                      | \\-            | $0.80f _{ptk}$ |
      | **允许开裂预应力构件**​  | 施工阶段    | $0.65f' _{ci}$           | \\-            | $0.90f _{ptk}$ |
      |                 | 成桥      | $0.45f' _{ck}$           | \\-            | \\-            |
      |                 | 使用组合I   | \$0.60\phi\_w f'\_{ck}\$ | \\-            | $0.80f _{ptk}$ |
      |                 | 使用组合III | \\-                      | \\-            | $0.80f _{ptk}$ |
      |                 | 其他使用组合  | \\-                      | \\-            | $0.80f _{ptk}$ |
      | **钢筋混凝土构件**​    | 施工阶段    | $0.65f' _{ci}$           | \\-            | \\-            |
      |                 | 成桥      | $0.45f' _{ck}$           | \\-            | \\-            |
      |                 | 使用组合I   | \$0.60\phi\_w f'\_{ck}\$ | \\-            | \\-            |
      |                 | 使用组合III | \\-                      | \\-            | \\-            |
      ...
      - 混凝土压应力限值（成桥、使用组合Ⅰ）：
        ...
        | 位置                      | 应力限值                     |
        | ----------------------- | ------------------------ |
        | 有效预应力和永久荷载共同引起          | $0.45f' _{ck}$           |
        | 有效预应力+永久荷载+临时荷载+运输/吊装引起 | \$0.60\phi\_w f'\_{ck}\$ |
  ...
  - 疲劳极限状态应力计算
    ...
    - 其他&#x20;
      - 对于除节段施工桥梁外的预应力构件，由疲劳组合I产生的压应力与未折减有效预应力和永久荷载之和的一半叠加后，在预应力损失后不得超过混凝土抗压强度的0.40倍（$0.40{f'_c}$）。（$\sigma_{\text{压}}= 疲劳I组合压应力 + 0.5×(未折减有效预应力+永久荷载) \leq 0.40 f'_c   $  ）
    ...
    - **柱**
      ...
      - 偏心距增大计算方法
        - 短柱计算规则 ($l_e/h < 12$) &#x20;
          - a) 单轴弯曲增大偏心距 &#x20;
            $$
            e_{\text{增大}} = \min\left(0.05h,\, 0.02\right) \, \text{m}
            $$
          - b) 双轴弯曲增大偏心距 &#x20;
            $$
...
- 计算说明
  ...
  - 预应力混凝土构件
    ...
    - 计算路径选择
      ...
      若$极限荷载引起的弯矩M \leq 开裂弯矩M_{cr}$，按弯曲未开裂状态计算承载力，否则按弯曲开裂状态计算承载力。
    - 弯曲未开裂状态计算
    ...
    - 4.1.3.2.2 弯曲开裂状态计算
      ...
      - 1）全预应力和A类构件 &#x20;
        ...
        | 关键参数      | 定义                 | 注意                                |
        | --------- | ------------------ | --------------------------------- |
        | \$d\$     | 受压边缘到钢筋束质心的距离      | *计算*钢筋束质心*不考虑弯起钢筋和钢束（规范没有明确这点）* ​ |
...
- 计算说明
  ...
  - 对于实心矩形截面、T型梁和其它实心截面形且不带凹角的梁腹的裂缝宽度计算
    - 计算公式： &#x20;
      ...
      | 符号                          | 物理意义                  | 备注                                                                                                              |
      | --------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------- |
      | $a _{\mathrm{cr}}$          | 开裂点（最大拉应变点）到最近钢筋表面的距离 | \\-                                                                                                             |
      | $c _{\mathrm{nom}}$         | 最外层钢筋净保护层厚度           | **用户在分析设置中设置“净保护层厚”** ​                                                                                         |
      | $d _{\mathrm{c}}$           | 受压混凝土高度               | \$d\_c=0\$时使用下方全截面受拉时的裂缝宽度计算公式计算                                                                                |
      | \$h\$                       | 截面总高度                 | \\-                                                                                                             |
      | $\varepsilon _{\mathrm{m}}$ | 考虑开裂水平的计算应变           | $0 \leq \varepsilon _{\mathrm{m}} \leq \varepsilon_1$                                                           |
      | \$\varepsilon\_1\$          | 开裂点计算应变               | \\-                                                                                                             |
      | $b _{\mathrm{t}}$           | 受拉钢筋质心水平处的截面宽度        | \\-                                                                                                             |
      | \$a^{\prime}\$              | 受压面到裂纹宽度点的距离          | 受拉区高度（最大拉应力点到中心轴的距离）+受压区高度（最大压应力点到中心轴的距离）                                                                       |
      | $M _{\mathrm{g}}$           | 永久荷载引起的弯矩             | 恒载效应                                                                                                            |
      | $M _{\mathrm{q}}$           | 活载引起的弯矩               | 活载效应，荷载比$M_q/M_g$由**用户在分析设置中设置，** 处理规则见荷载比$M_q/M_g$处理规则                                                         |
      | $\varepsilon _{\mathrm{s}}$ | 受拉钢筋计算应变              | \\-                                                                                                             |
      | $A _{\mathrm{s}}$           | 受拉钢筋有效面积              | 如果设计力矩的轴线和抵抗该力矩的受拉钢筋的方向彼此不垂直（例如在斜交板中）时：$A _{\mathrm{s}}=\Sigma\left(A_{\mathrm{t}} \cos^{4} \alpha _{1}\right)$ |
