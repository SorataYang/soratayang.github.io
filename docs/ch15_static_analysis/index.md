...
- **荷载分布宽度**
  ...
  - 荷载值：
    - 根据荷载位置的不同，规范要求不同。当无需考虑车轮分布重合，则结果表中的荷载值为 $\frac{P_{1}}{\text{单轮分布宽度}a}$ ；
    - 当需要考虑车轮分布重合，荷载值为按照单轮分布宽度a和双轮分布宽度a分别计算荷载值，即 $\frac{P_{1}}{\text{单轮分布宽度}a}$ 与 $\frac{P_{2}+P_{3}}{\text{双轮分布宽度}a}$ 作比较，两者取大值。
    - 当车轮位于支承处或支承附近处：无需考虑车轮分布重合；
    - 当车轮位于跨中位置：需考虑车轮分布重合；
    - 当车轮位于悬臂位置：$lc ≤ 2.5m$ 时，参考AASHTO 规范中对于悬臂板荷载分布宽度的计算公式，考虑车轮分布重合；$lc ≤ 2.5m$ 时，规范推荐采用附录沙柯公式，该公式直接计算弯矩值，故软件显示的荷载分布宽度由弯矩值反算出来，不涉及是否考虑车轮分布宽度重合的问题。
    - 荷载比例系数= $\frac{\text{荷载值}}{\text{最大单轮重}P_{1}}$。该值用于填入结构计算中定义“横向移动荷载>车道线”中，荷载比例系数表。同时在“横向移动荷载>车辆定义”中，车轮荷载需输入此处的最大单轮重P1，才可正确进行横向影响线加载。
