...
- 自定义
  ...
  - 设计基准风速（有车）：若用户不自定义，程序按照 $U_{d}=k_{f}(\frac{\text{基准高度}}{\text{车离水面高}})^{α_{0}}U_{z} $ 公式计算。
  - 设计基准风速（极限）：若用户不自定义，程序按照 $U_{d}=k_{f}(\frac{\text{基准高度}}{10m})^{α_{0}}U_{s10} $ 公式计算。
...
- 自定义
  ...
  若需要修改某个参数，需点击左侧列表中对应参数的勾选框，勾选后可修改表格对应列的值，取消勾选则恢复默认值。
  - 单根主缆外直径：若用户不自定义，程序参考经验公式 $D=2\sqrt{\frac{\text{单元面积}/\text{主缆单元组合数}}{0.8\pi}}$ 计算外直径。
  - 基准高度：若用户不自定义，该值为单元IJ端截面偏心点在z方向上高度+整体坐标系原点离水面高。
  - 设计基准风速（有车）：若用户不自定义，程序按照 $U_{d}=k_{f}(\frac{\text{基准高度}}{\text{车离水面高}})^{α_{0}}U_{z} $ 公式计算。
  - 设计基准风速（极限）：若用户不自定义，程序按照 $U_{d}=k_{f}(\frac{\text{基准高度}}{10m})^{α_{0}}U_{s10} $ 公式计算。
